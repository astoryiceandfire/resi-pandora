...
import Sideritemact from 'src/view/activity/sideritemact.jsx';
...
class Carouselpage extends React.Component {
    ...
    let siderItemx = siderItems.map((arrs,index)=>{
          return (<div key={`${type} + ${index}`} className='slide'>{arrs.map((item,index)=>(<Sideritemact key={item.id} type={type} slidecount={3} item={item} />))}</div>)
        })
          this.setState({
