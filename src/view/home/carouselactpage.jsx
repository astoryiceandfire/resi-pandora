...
import Sideritemact from 'src/view/home/sideritemact.jsx';
...
class Carouselpage extends React.Component {
    ...
    let siderItemx = siderItems.map((arrs,index)=>{
          return (<div key={`${categoryid} + ${index}`} className='slide'>{arrs.map((item,index)=>(<Sideritemact key={item.id} slidecount={3} item={item} />))}</div>)
        })
          this.setState({
